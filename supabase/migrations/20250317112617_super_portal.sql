/*
  ...
  1. Changes
    - Fix position calculation for moving notes
    - Add proper position validation
    - Add position normalization
    - Add detailed logging
    
  2. Details
    - Ensures notes move one step at a time
...
BEGIN
  ...
  IF v_old_parent_id IS NOT DISTINCT FROM p_new_parent_id THEN
    ...
    -- Ensure we only move one position at a time
    IF p_new_position > v_old_position THEN
      p_new_position := v_old_position + 1;
