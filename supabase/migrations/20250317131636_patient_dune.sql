/*
  # Fix note movement to prevent position conflicts
  
  1. Changes
    - Use temporary position to avoid unique constraint violations
    - Fix position updates for same-parent moves
    - Add proper position validation
    - Maintain unique positions
    
  2. Details
    - Uses temporary position to avoid conflicts
...
DECLARE
  ...
  v_temp_position integer := -999999; -- Use a very low temporary position
BEGIN
  ...
  -- First move the note to a temporary negative position to avoid conflicts
  UPDATE notes
