/*
  ...
  1. Changes
    ...
    - Use hierarchical path for proper ordering
    - Maintain parent-child relationships
    - Preserve position values
    
  2. Details
    - Uses path-based ordering for hierarchy
    - Ensures child notes stay with parents
    - Maintains position values at each level
*/
...
BEGIN
  ...
  WITH RECURSIVE note_hierarchy AS (
    -- Base case: Get root level notes ordered by position
    SELECT 
      id,
      content,
      parent_id,
      ...
      url_display_text,
      0 as depth,
      LPAD(position::text, 10, '0') as path,
      ARRAY[position] as position_path
    FROM notes
    WHERE project_id = source_id
    ...
    -- Recursive case: Get child notes ordered by position within each parent
    SELECT 
      c.id,
      c.content,
      c.parent_id,
  ...
  SELECT
    ...
    target_user_id
  FROM note_hierarchy
  ORDER BY path -- Ensures correct hierarchical order
