/*
  # Fix note count in copied projects

  1. Changes
    - Calculate note count after copying all notes
    - Update settings table with actual count
    - Add trigger to maintain note count
    - Add function to update project metadata

  2. Details
    - Ensures accurate note count
    - Maintains count automatically
    - Updates count on note changes
*/
...
RETURNS TRIGGER AS $$
BEGIN
  -- Update last_modified_at and note_count
  UPDATE settings
  SET 
...
DROP TRIGGER IF EXISTS update_project_last_modified_trigger ON notes;

-- Create trigger for project metadata updates
...
-- Function to copy a project with proper note count
...
BEGIN
  ...
  -- Copy project settings (note_count will be updated by trigger)
