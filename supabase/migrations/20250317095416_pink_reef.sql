/*
  # Fix note count when copying projects
  
  1. Changes
    - Update copy_project function to calculate actual note count
    - Add proper error handling
    - Maintain data integrity
    
  2. Details
    - Ensures accurate note count after project copy
...
-- Create improved function with proper note count handling
...
BEGIN
  ...
  SELECT 
    ...
    validated_title,
    description,
    v_note_count, -- Use actual count instead of copying old value
    last_level
  FROM settings
  WHERE id = source_id
