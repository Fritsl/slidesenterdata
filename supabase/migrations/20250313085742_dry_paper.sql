/*
  # Fix note sequence handling

  1. Changes
    - Add transaction control to move_note function
    - Add explicit locking to prevent concurrent modifications
    - Ensure sequences are always created for new notes
    - Fix sequence gaps and duplicates

  2. Details
    - Uses explicit transaction boundaries
    - Locks tables in correct order to prevent deadlocks
    - Handles edge cases like missing sequences
*/

-- Drop existing move_note function
DROP FUNCTION IF EXISTS move_note;

-- Create improved move_note function with transaction control
  ...
  BEGIN
    ...
    BEGIN
      ...
      IF v_project_id IS NULL THEN
        ...
        -- Get max sequence at current level
